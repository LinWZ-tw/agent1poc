...
<p style="font-size:12px;color:var(--muted);">
  Generated by the bioinformatics pipeline agent &mdash; run <code>{run_id}</code><br>
  Reproducibility:
  <a href="../reproduce.sh">reproduce.sh</a> &bull;
  <a href="../Snakefile">Snakefile</a> &bull;
  <a href="../methods.md">methods.md</a>
</p>
</main>
</body>
</html>"""
...
def run(
    *,
    ...
    # Generate reproducibility artifacts (shell script, Snakefile, methods.md)
    export_result: dict[str, Any] = {}
    try:
        export_result = _export.export_workflow(run_id)
        state.append_log(run_id, {"event": "workflow_exported", **export_result})
        _emit(
            type="system",
            text=f"Reproducibility files written: reproduce.sh · Snakefile · methods.md → {state.run_dir(run_id)}",
            agent=_LABEL,
        )
    except Exception as exc:  # noqa: BLE001
        state.append_log(run_id, {"event": "export_error", "error": str(exc)})
    ...
    return {"report_md": str(md_path), "report_html": str(html_path), **export_result}
